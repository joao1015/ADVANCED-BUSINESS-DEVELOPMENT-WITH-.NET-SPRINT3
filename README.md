...
-----
Aqui está o seu README formatado em Markdown.
...
✅ O que esta API entrega (Rubrica)
3 entidades: Motos, Tags, Anchors

CRUD completo e status codes adequados

Versionamento de API (/api/v1/...)

Segurança com JWT (login + Authorization: Bearer)

Health Check (verifica Mongo)

Endpoint com ML.NET (estimativa de distância a partir do RSSI)

Swagger/OpenAPI com exemplos

README com instruções de execução e testes

⚠️ Observação: nesta Sprint não há EF Core. O banco é MongoDB (Atlas ou local).

🧱 Arquitetura & Principais Pacotes
ASP.NET Core 8 Web API

MongoDB.Driver

Asp.Versioning (versionamento)

Swashbuckle.AspNetCore (Swagger)

Microsoft.ML (ML.NET – regressão simples para estimar distância por RSSI)
...
/Models
/Services
  /ML  (MLEstimatorService.cs)
/Swagger (exemplos opcionais)
/Properties/launchSettings.json
Program.cs
appsettings.json
⚙️ Configuração
...
Em Network Access, adicione o seu IP (ou 0.0.0.0/0 só para teste).

Crie Database User e use a string mongodb+srv://<user>:<pass>@cluster0.XXXX.mongodb.net/?appName=Cluster0

No appsettings.json, troque "ConnectionString" por essa string do Atlas.

Dica: no Atlas, o header SNI/TLS é obrigatório. Use drivers recentes e não coloque aspas no token do header Authorization.
...
🩺 Health Check (10 pts)
Verifica conectividade ao MongoDB.
...
{ "status": "Healthy", "utc": "2025-10-27T23:10:23.6789198Z" }
Se falhar, retorna 503/500 com motivo.

🔐 Autenticação (JWT) (25 pts)
Login
...
{ "usuario": "jp", "senha": "123" }
Resposta 200: string JWT (sem JSON, é a string pura).

Importante: use o token sem aspas no header: Authorization: Bearer <TOKEN_AQUI>
